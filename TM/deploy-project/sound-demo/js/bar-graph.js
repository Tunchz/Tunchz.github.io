...
let predicted_result = {label:"", color:"#FFFFFF", conf: 0},
    prdictedResultEl = null;
...
export async function setupBarGraph(URL) {
    ...
    // get the element for predicted result
    prdictedResultEl = document.getElementById('predicted-result');
...
function makeBar(label, index) {
    ...
    bars.push({
        ...
        percent: percentEl,
        color: color,
        label: label == '_background_noise_'?'background noise':label
    });
}
...
export function updateBarGraph(data) {
    ...
    data.forEach((probability, index) => {
        ...
        if (probability > predicted_result.conf) {
            predicted_result = {
                conf: probability,
                label: bars[index]?.label || "?",
                color: bars[index]?.color || "#000000"
            }
        }
    
    });

    prdictedResultEl.innerHTML = predicted_result.label
    prdictedResultEl.style.color = predicted_result.color;
}
